...
export class HandStatusHUD{
  constructor(bus){this.bus=bus;this.node=null;this.roles={primary:"Right",control:"Left"};this.input="hands"}
  mount(root){
  ...
  render(hands){
    ...
    list.textContent=hands.sort((a,b)=>a.role==="primary"?-1:1).map(h=>`${h.role.toUpperCase()} · ${h.name.toUpperCase()} · ${h.far?"FAR":"NEAR"}`).join("   |   ");
  }
}
