...
export class HandTracking{
  constructor({video,bus}){
    ...
    this.missStreak=0;this.frameIndex=0;this.zoomCanvas=document.createElement("canvas");this.zoomCanvas.width=960;this.zoomCanvas.height=540;this.zoomCtx=this.zoomCanvas.getContext("2d",{alpha:false});
  }
  async init(){
  ...
  sourceForFrame(){
    const useZoom=this.missStreak>10&&this.frameIndex%2===0;if(!useZoom)return{source:this.video,crop:null,farAssist:false};
  ...
  loop=()=>{
    if(this.video.readyState>=2&&this.video.currentTime!==this.lastVideoTime){
      ...
      if(landmarks.length)this.missStreak=0;else this.missStreak++;
