...
export class CalibrationEngine{
  ...
  capture(hands){
    ...
    for(const h of hands){
      ...
      if(handSize>.015&&handSize<.5)this.samples.push(handSize);
  ...
  finish(){
    const avgHand=average(this.samples.slice(-180))||.10,sortedPinch=[...this.pinches].sort((a,b)=>a-b),p20=sortedPinch[Math.floor(sortedPinch.length*.2)]||.045;
    ...
    this.profile={
      ...
      deadZone:Number(clamp(avgHand*.34,.025,.075).toFixed(4)),minMotionPx:Math.round(clamp(avgSpeed*22,10,28)),
      holdMs:Math.round(clamp(360-avgSpeed*45,250,380)),longHoldMs:1550,smoothing:Number(clamp(.31-avgSpeed*.04,.19,.31).toFixed(3)),
